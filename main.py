...
OCR_TIMEOUT_SECONDS = 15            # Slightly higher to avoid Render timeouts
...
def extract_ocr_lines(image: Image.Image) -> List[Dict]:
    """
    OCR (reduced work):
    - Assumes user oriented the image so titles read left-to-right.
    - Runs ONLY whole-image OCR (no slicing) to avoid Render timeouts.
    ...
    # Whole-image OCR only (quickest / most reliable on Render)
